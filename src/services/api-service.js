  ...
  sendApplication(formState) {
    ...
    const jsonPayload = {
      ...
      firstName: formState.ahFirstName
        ? formState.ahFirstName.substring(0, firstNameMaxLength)
        : null,
      ...
      country: formState.resCountry
        ? replaceSpecialCharacters(formState.resCountry).substring(0, 25)
        : null,
    ...
    if (!applyingForMSP) {
      ...
      jsonPayload.city = formState.mailCity
        ? replaceSpecialCharacters(formState.mailCity).substring(0, 25)
        : null;
      ...
      jsonPayload.country = formState.mailCountry
        ? replaceSpecialCharacters(formState.mailCountry).substring(0, 25)
        : null;
    }
    ...
    if (formState.hasSpouse === "Y") {
      ...
      jsonPayload.spouse = {
        firstName: formState.spouseFirstName
          ? formState.spouseFirstName.substring(0, firstNameMaxLength)
          : null,
        lastName: formState.spouseLastName
          ? formState.spouseLastName.substring(0, lastNameMaxLength)
          : null,
    ...
    if (formState.isApplyingForFPCare) {
      const postalCode =
        formState.isMailSame && formState.resPostalCode
          ? stripSpaces(formState.resPostalCode)
          : stripSpaces(formState.mailPostalCode);
      const persons = [];
    ...
    if (formState.isApplyingForSuppBen) {
      ...
      if (
        formState.selectedDisabilityRecipients &&
        formState.selectedDisabilityRecipients.includes("spouse")
      ) {
        numDisabled += 1;
      }
      ...
      if (
        formState.selectedDisabilityRecipients &&
        formState.selectedDisabilityRecipients.includes("ah")
      ) {
        numDisabled += 1;
      }
      ...
      jsonPayload.supplementaryBenefits = {
        ...
        disabilityDeduction:
          parseInt(formState.ahDisabilityCreditDeduction) +
